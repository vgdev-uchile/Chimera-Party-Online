...
onready var target = initial_position.y
...
#		close()
	if position.y != target:
		position.y = lerp(position.y, target, 0.25)

func open():
	rpc("sync_open")

sync func sync_open():
	target = initial_position.y - $CollisionShape2D.shape.extents.y * 2

func close():
	rpc("sync_close")

sync func sync_close():
	target = initial_position.y
